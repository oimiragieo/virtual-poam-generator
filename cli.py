...
Examples:
  ...
  karp-clone report.nessus --summary -o summary.csv
    ...
    parser.add_argument(
        'input_file',
        help='Input .nessus file to process'
    )
    ...
    parser.add_argument(
        '--summary',
        action='store_true',
        help='Export summary only (CSV format)'
    )
    ...
    try:
        ...
        if args.verbose:
            print(f"Found {report.total_hosts} hosts with {report.total_vulnerabilities} vulnerabilities")
        ...
        if args.summary:
            ...
            if args.report_type == 'vulnerability':
                ...
                print(f"Error: Unsupported report type '{args.report_type}' for Excel format")
                sys.exit(1)
        elif args.format == 'html':
            if args.verbose:
                print(f"Exporting {args.report_type} HTML report...")
            output_path = export_html_report(analysis_data, args.output, args.template_dir)
        elif args.format == 'pdf':
            if args.verbose:
                print(f"Exporting {args.report_type} PDF report...")
            output_path = export_pdf_report(analysis_data, args.output, args.template_dir)
